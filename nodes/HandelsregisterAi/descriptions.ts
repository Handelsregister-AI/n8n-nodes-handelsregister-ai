...
export const nodeProperties: INodeProperties[] = [
  {
    ...
    options: [
      ...
      },
      {
        name: 'Get Signal',
        value: 'getSignal',
        description: 'Get one company-change Signal by its stable event ID',
        action: 'Get a signal',
      },
      {
        name: 'Get Signal Catalog',
        value: 'getSignalCatalog',
        description: 'Get the public Signals topic catalog',
        action: 'Get the signal catalog',
      },
      {
        name: 'List Signals',
        value: 'listSignals',
        description: 'List and filter company-change Signals with cursor pagination',
        action: 'List signals',
      },
      {
  ...
  },
  {
    ...
    description: 'Stable event ID returned by List Signals',
    displayOptions: { show: { operation: ['getSignal'] } },
    required: true,
  },
  {
    displayName: 'Topics',
    ...
    options: [
      {
        ...
        value: 'CAPITAL_CHANGES',
        description: 'Changes to share, liable, nominal, or authorized capital',
      },
      {
        name: 'Closures and Dissolutions',
        value: 'CLOSURES',
        description: 'Dissolution, liquidation, deletion, or expiry',
      },
      {
        name: 'Insolvencies (Pro or Max)',
        value: 'INSOLVENCIES',
        description: 'Openings, protective measures, and completed proceedings',
      },
      {
        name: 'Master Data Changes',
        value: 'MASTER_DATA_CHANGES',
        description: 'Company name, seat, address, or registry changes',
      },
      {
        name: 'New Registrations',
        value: 'NEW_REGISTRATIONS',
        description: 'Organizations newly registered in the commercial register',
      },
      {
        name: 'Role Holder Changes',
        value: 'ROLE_HOLDER_CHANGES',
        description: 'Changes to management, board, and procuration roles',
      },
      {
        name: 'Transformations and M&A (Max)',
        value: 'TRANSFORMATIONS',
        description: 'Mergers, divisions, conversions, transfers, and enterprise agreements',
      },
    ],
    default: [],
    description: 'Topics to include. Leave empty to include all entitled topics.',
    displayOptions: { show: { operation: ['listSignals'] } },
  },
  {
    displayName: 'Organization IDs',
    name: 'signalOrganizationIds',
    type: 'string',
    default: '',
    placeholder: 'e.g., organization-ID-one, organization-ID-two',
    description: 'One or more organization entity IDs separated by commas (OR semantics)',
    displayOptions: { show: { operation: ['listSignals'] } },
  },
  {
    displayName: 'From',
    ...
    type: 'dateTime',
    default: '',
    description: 'Inclusive lower bound for the publication date',
    displayOptions: { show: { operation: ['listSignals'] } },
  },
  {
    displayName: 'To',
    ...
    type: 'dateTime',
    default: '',
    description: 'Inclusive upper bound for the publication date',
    displayOptions: { show: { operation: ['listSignals'] } },
  },
  {
    displayName: 'Return All',
    name: 'signalsReturnAll',
    type: 'boolean',
    default: false,
  ...
  },
  {
    ...
    name: 'signalsMaxResults',
    type: 'number',
    typeOptions: { minValue: 0 },
    default: 0,
    description:
      'Maximum Signals to return when Return All is enabled. Use 0 for no client-side cap.',
    displayOptions: {
      show: { operation: ['listSignals'], signalsReturnAll: [true] },
    },
  },
  {
    displayName: 'Cursor',
    ...
    type: 'string',
    default: '',
    description: 'Opaque next_cursor from a previous response; send it unchanged',
    displayOptions: {
      show: { operation: ['listSignals'], signalsReturnAll: [false] },
    },
  },
  {
    displayName: 'Output',
    ...
    options: [
      {
        ...
        value: 'response',
        description: 'Return Signals, pagination, filters, warnings, and metadata in one item',
      },
      {
        name: 'One Item per Signal',
        value: 'split',
        description: 'Return every Signal as a separate n8n item',
      },
    ],
    default: 'split',
    displayOptions: { show: { operation: ['listSignals'] } },
